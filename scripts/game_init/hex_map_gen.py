...
class WorldGenerator:
    ...
    def _generate_islands(self):
        self._generate_island_centers()
        self._generate_island_shapes() 

    def _generate_island_centers(self):
        """
        ...
        engine = qmc.PoissonDisk(
            d=2, radius=self.config.i_min / max(self.config.cols-2*self.config.border, self.config.rows-2*self.config.border), 
            rng=self.rng, ncandidates=self.config.num_islands
        )
        sample = engine.fill_space()


    def _generate_island_shapes(self):
        pass
    ...
    def show_map(self) -> None:
        ...
        norm   = pltc.BoundaryNorm(boundaries=[v - 0.5 for v in cvals] + [cvals[-1] + 0.5], ncolors=len(cvals))

        fig, ax = plt.subplots(figsize=(self.config.cols * 0.45, self.config.rows * 0.45))
        ax.set_aspect("equal")
        ax.set_axis_off()
        ...
        for (q, r), tile in self.grid.items():
            ...
            patches.append(RegularPolygon((x, y), numVertices=6, radius=hex_r*1.00, orientation=0))
            values.append(tile)
        
        collection = PatchCollection(patches, cmap=cmap, norm=norm, edgecolor="darkgray", linewidth=0.5)
        ...
        # Legend
        legend_elements = [
            mpatches.Patch(facecolor=TRANSLATOR[t]["color"], label=TRANSLATOR[t]["label"])
            for t in [0, 1, 2, 3, 4]
        ]
        ax.legend(
...
def parse_args():
    ...
    parser.add_argument("--i_max", type=int, default=20)
